...
public class InstallScannerTests
{
    ...
    public void Reports_a_pack_whose_directory_is_present_as_installed()
    {
        ...
        var result = Assert.Single(InstallScanner.Scan(temp.Path, new[] { Pack("EP01") }));
    ...
    public void Reports_a_pack_missing_one_of_its_directories_as_partial()
    {
        ...
        var result = Assert.Single(InstallScanner.Scan(temp.Path, new[] { Pack("SP30", "SP30", "SP30_Delta") }));
    ...
    public void Compares_directory_names_and_not_the_whole_path()
    {
        ...
        var result = Assert.Single(InstallScanner.Scan(game, new[] { Pack("SP20") }));
    ...
    public void Reports_every_pack_as_not_installed_when_the_game_directory_is_absent()
    {
        ...
        var results = InstallScanner.Scan(Path.Combine(temp.Path, "absent"), new[] { Pack("EP01"), Pack("EP02") });

        Assert.Equal(2, results.Count);
        Assert.All(results, r => Assert.Equal(PackInstallState.NotInstalled, r.State));
    }
    ...
    public void Keeps_the_order_of_the_catalog()
    {
        ...
        var results = InstallScanner.Scan(temp.Path, new[] { Pack("EP02"), Pack("EP01") });

        Assert.Equal(new[] { "EP02", "EP01" }, results.Select(r => r.Code));
    }
    ...
    public void Reports_every_pack_as_not_installed_when_the_game_directory_cannot_be_read()
    {
        ...
        try
        {
            ...
            var results = InstallScanner.Scan(gameDir, new[] { Pack("EP01"), Pack("EP02") });
